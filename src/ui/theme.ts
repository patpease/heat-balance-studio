 ...
 * palette is defined three times in `tokens.css`: light on `:root`, dark inside
...
const STORAGE_KEY = 'heat-balance-studio:theme';
 ...
 *
 * `resolved` is what the interface is actually painting, which is what decides
 * which of the two buttons reads as pressed. The section drawing and the chart
 * are SVG and inherit the tokens directly, so unlike the sibling this app has
 * nothing that needs to be TOLD the palette.
 */
